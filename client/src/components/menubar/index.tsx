...
@connect((state) => {
  ...
  return {
    ...
    enableReembedding:
      (state as any).config?.parameters?.["enable-reembedding"] ?? false,
  };
})
class MenuBar extends React.PureComponent<{}, State> {
  ...
  render() {
    const {
      ...
      subsetResetPossible,
      // @ts-expect-error ts-migrate(2339) FIXME: Property 'enableReembedding' does not exist on typ... Remove this comment to see the full error message
      enableReembedding,
